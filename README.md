...
本仓库另有 `api-approach` 分支，用于尝试把前端线稿提取从本地 XDoG/图像处理流程替换为图像 API 生成的语义线稿。

- 分支地址：[api-approach](https://github.com/Sirin-p7/DFT-Edge-extraction-and-rotating-arm-diagram/tree/api-approach)
- 主分支 `main`：保留纯本地图像处理流程，使用 `XDoG_Guide` / `XDoG_Support` 双通道生成线稿。
- `api-approach`：调用 OpenAI-compatible 图像接口生成 `api_line_raw.png`，再在本地二值化为单通道 `API_Line`，后续继续复用 SVG component 拆分、DFT 预运算和网页旋臂查看器。
- 适用场景：当本地 XDoG 对复杂动漫图、弱边缘、阴影或材质纹理的取舍不稳定时，可以用 API 分支获得更语义化的 manga ink line art，再进入同一套 Fourier 旋臂流程。

`api-approach` 分支包含 Windows x64 预编译包说明，并内置 `potrace.exe`、OpenCV DLL 和网页查看器；使用时只需要在包内 `api_line_config.ini` 填写自己的中转地址、密钥和模型名。
...
```mermaid
flowchart LR
    A[输入图片] --> B[尺寸归一化与低分辨率增强]
    B --> C[主体/角色 guide 估计]
    C --> D[XDoG 与边缘候选线稿]
    D --> E[噪声过滤与色块轮廓化]
    E --> F[Guide / Support 双通道]
    F --> G[potrace 转 SVG]
    G --> H[拆分 component SVG]
    H --> I[DFT 预运算]
    I --> J[网页播放旋臂绘制动画]
```
...
results_v2/<image_name>/
  XDoG_Guide.svg          主体关键线稿
  XDoG_Support.svg        支持区补充线稿
  comp/                   拆分后的连通组件 SVG
  dft_data/               Scene 模式使用的预运算 JSON
  FourierCoefficient/     component 傅里叶系数文本
```
...
<p align="center">
  <a href="results_v2/art/dft_anim/readme_epicycle.gif">
    <img src="results_v2/art/dft_anim/readme_epicycle.gif" alt="art 旋臂绘制动画" width="420">
  </a>
</p>
[如果预览器不播放动画，可以直接打开 GIF 文件。](results_v2/art/dft_anim/readme_epicycle.gif)

上面的动画使用 `art` 的完整线稿做浅色叠底，再叠加当前 component 的傅里叶旋臂、笔尖和已绘制轨迹。

打开本地 viewer 后，在 `View -> Scene` 中选择对应图片和通道，就可以查看预运算后的旋臂绘制效果。
